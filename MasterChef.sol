...
// File: contracts/BSCYtoken.sol

pragma solidity 0.6.12;

// BSCYToken with Governance.
contract BSCYtoken is ERC20("BSC Yield Farm", "BSCY"), Ownable {
...
// File: contracts/BSCYMasterChef.sol

pragma solidity ^0.6.0;

contract BSCYMasterChef is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        BSCYtoken rewardToken;        // Address of reward token contract.
...
constructor(
    ...
    function _setAllocPoints(BSCYtoken _rewardToken, uint256 _allocPoint) internal onlyOwner {
    ...
    function claimReward(uint256 _pid) public {
        require(status == true, "BSCY::withdraw: can not claim reward");
        updatePool(_pid);
        _harvest(_pid);
    }
    ...
    // Deposit LP tokens to BSCY.
                                    function deposit(uint256 _pid, uint256 _amount, address _referrer) public {
        require(_amount > 0, "BSCY::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from BSCY.
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(status == true, "BSCY::withdraw: can not withdraw");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "BSCY::withdraw: not good");
    ...
    function totalLock(BSCYtoken _rewardToken) public view returns (uint256) {
